...
class PaymentService {
  constructor() {
    ...
    try {
      ...
      console.warn('Razorpay service initialization failed:', error.message);
      this.razorpay = null;
    }
    ...
    try {
      ...
      console.warn('PayPal service initialization failed:', error.message);
      this.paypal = null;
    }
  ...
  getAvailablePaymentMethods() {
    ...
    if (this.razorpay && this.razorpay.isEnabled && process.env.RAZORPAY_KEY_ID && process.env.RAZORPAY_KEY_SECRET) {
      methods.push({
        id: 'razorpay',
        name: 'Razorpay',
        type: 'gateway',
    ...
    if (this.stripe && this.stripe.isEnabled && process.env.STRIPE_SECRET_KEY) {
      methods.push({
        id: 'stripe',
        name: 'Stripe',
        type: 'gateway',
    ...
    if (this.paypal && this.paypal.isEnabled && process.env.PAYPAL_CLIENT_ID && process.env.PAYPAL_CLIENT_SECRET) {
      methods.push({
        id: 'paypal',
        name: 'PayPal',
        type: 'gateway',
  ...
  async createPaymentOrder(orderData) {
    try {
      ...
      switch (gateway) {
        case 'razorpay':
          if (!this.razorpay || !this.razorpay.isEnabled) {
            return {
              success: false,
              error: 'Razorpay service is not available. Please try another payment method.',
            };
          }
